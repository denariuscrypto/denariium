...
class BitcoinAverage(ExchangeBase):
    def get_rates(self, ccy):
        json = self.get_json('apiv2.bitcoinaverage.com', '/indices/global/ticker/DNR%s' % ccy)
    ...
    def historical_rates(self, ccy):
        history = self.get_csv('apiv2.bitcoinaverage.com',
                               "/indices/global/history/DNR%s?format=csv" % ccy)
...
class BitcoinVenezuela(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        rates = [(r, json['DNR'][r]) for r in json['DNR']
                 if json['DNR'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    def historical_rates(self, ccy):
        return self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=DNR")[ccy +'_LTC']
...
class GoCoin(ExchangeBase):
    def get_rates(self, ccy):
        ...
        ltc_prices = json['prices']['DNR']
        return dict([(r, Decimal(ltc_prices[r])) for r in ltc_prices])

class HitBTC(ExchangeBase):
    def get_rates(self, ccy):
        ccys = ['EUR', 'USD']
        json = self.get_json('api.hitbtc.com', '/api/1/public/DNR%s/ticker' % ccy)
        result = dict.fromkeys(ccys)
        if ccy in ccys:
...
class Kraken(ExchangeBase):
    ...
    def historical_rates(self, ccy):
        query = '/0/public/OHLC?pair=DNR%s&interval=1440' % ccy
...
class FxThread(ThreadJob):
    ...
    def get_fiat_status_text(self, btc_balance):
        rate = self.exchange_rate()
        return _("  (No FX rate available)") if rate is None else " 1 DNR~%s %s" % (self.value_str(COIN, rate), self.ccy)
