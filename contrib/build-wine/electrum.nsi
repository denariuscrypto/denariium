...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/denariium-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\denariium-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\denariium-${PRODUCT_VERSION}.exe" "" "$INSTDIR\denariium-${PRODUCT_VERSION}.exe" 0

  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\denarius" "" "URL:denarius Protocol"
  WriteRegStr HKCU "Software\Classes\denarius" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\denarius" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\denarius\shell\open\command" "" "$\"$INSTDIR\denariium-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\denarius"
